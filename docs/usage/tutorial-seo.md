...
to setup a multilingual website with MODx and Babel by using one domain and
...
I'll describe the procedure of setting up the a multilingual site by providing a
...
To make your multilingual site work properly you have to add some rewrite rules
to your .htaccess file which handle requests to your (physically non-existing)
language subfolders.
...
```
RewriteRule ^(en|de|es)/favicon.ico$ favicon.ico [L,QSA]
```

Ok, now your pages should be accessible via the language subfolders and linking
your pages with relative links should work, too.

But there is still a problem regarding relative links: linking assets like CSS,
